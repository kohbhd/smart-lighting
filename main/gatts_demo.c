...
#define GATTS_CHAR_UUID_TEST_A      0xFF02
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        read_current_lux(&val_lux);
        val_pir = gpio_get_level(GPIO_PIR_SENSOR);
    	ESP_LOGI("PIR", "PIR: %d", val_pir);
        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 4;
        rsp.attr_value.value[0] = (val_pir&0xff);//0x0e;
        rsp.attr_value.value[1] = (val_pir>>8)&0xff;//0x3c;
        rsp.attr_value.value[2] = val_lux/100;//0x2e;
        rsp.attr_value.value[3] = val_lux%100;//0xbf;
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_READ_EVT: {
        ...
        rsp.attr_value.handle = param->read.handle;
        rsp.attr_value.len = 4;
        rsp.attr_value.value[0] = (val_pir&0xff);//0x01;
        rsp.attr_value.value[1] = (val_pir>>8)&0xff; //0x02;
        rsp.attr_value.value[2] = val_lux/100;//0x03;
        rsp.attr_value.value[3] =  val_lux%100;//0x04;
        esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id,
                                    ESP_GATT_OK, &rsp);
        break;
    }
...
void app_main()
{
    ...
    //xTaskCreate(&pir_task, "pir_task", 512, NULL, 5, NULL);
