...
esp_err_t read_current_lux(long *val) {
	uint8_t data1_h = 0;
	uint8_t data2_h = 0;
	uint8_t data1_l = 0;
	uint8_t data2_l = 0;
	...
	ESP_LOGI("LUX", "1x value %ld", value);

	//uint16_t ch0 = (data1_l << 8) | data1_h;
	//uint16_t ch1 = (data2_l << 8) | data2_h;
	uint16_t ch0 = (data1_h << 8) | data1_l;
	uint16_t ch1 = (data2_h << 8) | data2_l;

	long lux = calculateLux(ch0, ch1);
	ESP_LOGI("LUX", "calculated lux value %ld", lux);
